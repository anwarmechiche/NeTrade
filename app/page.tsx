...
// Importation de img standard car Base64 peut être trop lourd pour Next Image sans config
import Link from "next/link"; 
import { supabase } from "@/lib/supabase";
import { 
  MessageCircle, ShieldCheck, Search, X, Send, Bot, 
  Heart, Share2, MoreHorizontal, ListFilter, Filter, UserCircle, Gavel
} from "lucide-react";
...
type Product = { 
  ...
  description: string; 
  price: number; 
  image: string | null; // Assure-toi que c'est bien 'image' pour correspondre à ta BDD
...
export default function AffarInstagramMarketplace() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      // On récupère tout (*) incluant la colonne 'image'
      const { data: pData } = await supabase.from("products").select("*");
      if (pData) { 
        setProducts(pData as Product[]); 
        setFilteredProducts(pData as Product[]); 
      }
      setLoading(false);
    };
    fetchData();
  ...
  return (
    <div className="min-h-screen bg-white text-slate-900 font-sans">
      ...
      <nav className="sticky top-0 z-50 bg-white/95 backdrop-blur-md border-b border-slate-100 px-4">
        <div className="max-w-xl mx-auto h-16 flex justify-between items-center">
          ...
          <div className="flex gap-4 items-center">
            <Link href="/login" className="flex items-center gap-2 bg-slate-900 text-white px-4 py-2 rounded-full text-xs font-bold hover:bg-blue-600 transition-colors shadow-md">
              <UserCircle size={18} /> Se connecter
            </Link>
          </div>
        </div>
      ...
      <main className="max-w-lg mx-auto py-2">
        {loading ? (
          <div className="flex justify-center py-20 animate-pulse text-slate-400 font-bold italic text-sm">Chargement du Marketplace...</div>
        ) : filteredProducts.map((product) => (
          <article key={product.id} className="mb-10 border-b border-slate-50 pb-4">
            <div className="flex items-center justify-between px-4 mb-3">
              <div className="flex items-center gap-3">
                <div className="w-9 h-9 rounded-full bg-blue-50 flex items-center justify-center font-black text-blue-600 text-xs">
                  {getMerchantName(product.merchant_id).substring(0,2).toUpperCase()}
                </div>
                <div>
            ...
            {/* ZONE IMAGE CORRIGÉE */}
            <div className="relative group aspect-square bg-slate-100 cursor-pointer overflow-hidden" 
                 onClick={() => {setActiveChat(product); setMessages([{ content: `Bonjour ! Parlons de : ${product.name}`, sender_type: 'bot' }]);}}>
              {product.image ? (
                <img 
                  src={product.image} 
                  alt={product.name} 
                ...
                <div className="w-full h-full flex items-center justify-center text-slate-300 italic text-xs">Pas d'image</div>
              )}
              
              <div className="absolute inset-0 bg-black/10 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center">
                <div className="bg-white px-6 py-3 rounded-full font-bold flex items-center gap-2 shadow-xl">
                  <Gavel size={18} className="text-blue-600" /> Négocier
                </div>
              </div>
              <div className="absolute bottom-4 right-4 bg-slate-900 text-white px-3 py-1.5 rounded-lg text-sm font-black shadow-lg">
                {product.price.toLocaleString()} DA
              </div>
            </div>

            <div className="px-4 py-4 flex gap-5">
              <MessageCircle size={28} onClick={() => {setActiveChat(product); setMessages([{ content: `Bonjour ! Parlons de : ${product.name}`, sender_type: 'bot' }]);}} className="cursor-pointer hover:text-blue-600 transition-colors" />
              <Heart size={28} className="hover:text-red-500 cursor-pointer transition-colors" />
              <Share2 size={26} className="hover:text-blue-400 cursor-pointer transition-colors" />
            </div>
      ...
      {activeChat && (
        <div className="fixed inset-0 z-[100] flex items-end justify-center bg-black/60 backdrop-blur-sm p-4">
          <div className="w-full max-w-md h-[80vh] bg-white rounded-t-[2.5rem] flex flex-col shadow-2xl animate-in slide-in-from-bottom duration-300 overflow-hidden">
            <div className="p-6 border-b flex justify-between items-center bg-white">
              ...
              <button onClick={() => {setActiveChat(null); setMessages([]);}} className="bg-slate-100 p-2 rounded-full hover:bg-slate-200 transition-colors"><X size={20}/></button>
            </div>

            <div className="flex-1 overflow-y-auto p-6 space-y-4 bg-slate-50/50">
              {messages.map((msg, i) => (
                <div key={i} className={`flex ${msg.sender_type === 'client' ? 'justify-end' : 'justify-start'}`}>
                  <div className={`max-w-[85%] p-3.5 rounded-2xl text-[13px] ${msg.sender_type === 'client' ? 'bg-blue-600 text-white rounded-br-none' : 'bg-white border border-slate-100 text-slate-700 rounded-bl-none shadow-sm'}`}>
                    {msg.content}
                  </div>
                </div>
              ))}
            ...
            <div className="p-6 border-t bg-white">
              <div className="flex items-center gap-2 bg-slate-100 rounded-2xl px-4 py-3">
                ...
                />
                <button onClick={handleSendMessage} className="text-blue-600 font-bold hover:scale-110 transition-transform"><Send size={22} /></button>
              </div>
            </div>
          </div>
        </div>
