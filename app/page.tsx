  ...
  useEffect(() => {
    const setupPush = async () => {
      try {
        ...
        // 2. Si pas encore demandé, on demande la permission
        if (perm.receive === 'prompt') {
          perm = await PushNotifications.requestPermissions();
        }

        // 3. Si l'utilisateur accepte, on enregistre l'appareil sur Firebase
        if (perm.receive === 'granted') {
          await PushNotifications.register();
        }

        // 4. Écouter le succès de l'enregistrement (Token)
        PushNotifications.addListener('registration', (token) => {
          console.log('Push Registration Success. Token:', token.value);
          // Optionnel : Sauvegarder ce token dans Supabase plus tard pour cibler cet utilisateur
        });
        ...
        console.log('Notifications non disponibles (probablement sur navigateur web)');
      }
    };
